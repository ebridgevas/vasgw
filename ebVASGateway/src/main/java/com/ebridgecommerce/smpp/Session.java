...
public class Session extends SmppObject {
	...
	/**
	 * Status of bounding. It's set by <code>bind</code> methdod if the binding is
	 * successfull.
	 * 
	 * @see #bind(BindRequest)
	 */
	 ...
	 * current implementation of session, so it's currently altered to reflect the
	 * impplementation.
	 * 
	 * @see #initialiseStateMatrix()
	 */
	 ...
	 * compatiblity we leave it set to false.
	 */
	 ...
	 * PDU if you are in wrong state) and it'll discover bad logic in your
	...
	/**
	 * Indicates that the session is session for MC 'point of view.' MC is Message
	 * Center and it is a term for generic Message Centre such as SMSC.
	 * 
	 * @see #type
	 */
	 ...
	 * routing entity, simply the other side of normal ESME). Normally this would
	 * be equal to TYPE_ESME as mostly this library will be used for developing
	 * ESME applications and not MCs. If you don't understand what is this
	...
	/**
	 * If the receiving is asynchronous, <code>pduListener</code> must contain the
	 * callback object used for processing of PDUs received from the SMSC.
	 ...
	 * passed to the <code>pduListener</code> object in the <code>receiver</code>.
	 * 
	 * @see #pduListener
	 * @see #setServerPDUEventListener(ServerPDUEventListener)
	 * @see #bind(BindRequest)
	...
	/**
	 * Default constructor made protected as it's not desirable to allow creation
	 * of <code>Session</code> without providing <code>Connection</code>.
	 */
	protected Session() {
	}
	 ...
	 * In most cases the <code>connection</code> parameter will be an instance of
	 * <code>TCPIPConnection</code> class.
	 * 
	 * @param connection
	...
	/**
	 * Opens the connection for communication. Sets indication that the connection
	 * is opened.
	 * 
	 ...
	 * <code>Receiver</code> from the SMSC. Note that this method implicitly sets
	 * asynchronous type of receiving.
	 *
	 * @param pduListener
	...
	/**
	 * Sets the type of the session. The type can be either ESME or MC viewed from
	 * the side where the instance of the <code>Session</code> is used. Set the
	 * type of the session before opening the session.
	 *
	 * @param type
	 ...
	 * provided. On details about bind see <a
	 * href="#bind(BindRequest,ServerPDUEventListener)"
	 * >bind(BindRequest,ServerPDUEventListener)</a>.
	 *
	 * @see #bind(BindRequest,ServerPDUEventListener)
	 */
	final public BindResponse bind(BindRequest bindReq) throws ValueNotSetException, TimeoutException, PDUException, IOException, WrongSessionStateException {
		checkState(bindReq);
	 ...
	 * <code>pduListener</code>. If the bind to SMSC isn't successfull, the thread
	 * is stopped and the connection is closed.<br>
	 ...
	 * @see BindTransciever
	 */
	final public BindResponse bind(BindRequest bindReq, ServerPDUEventListener pduListener) throws ValueNotSetException, TimeoutException, PDUException,
			IOException, WrongSessionStateException {
		checkState(bindReq);
		if (bound) {
	 ...
	 * this operation is only allowed when your session acts as SMSC (MC) session.
	 * ESMEs can't send outbinds.
	 * <p>
	 ...
	 *              rest of data not received for too long time
	 * @exception ValueNotSetException
	 *              optional param not set but requested (this exception doesn't
	 *              apply for outbind)
	 * @see Outbind
	 * @see OutbindReceiver
	 */
	final public void outbind(Outbind request) throws ValueNotSetException, TimeoutException, PDUException, IOException, WrongSessionStateException {
		checkState(request);
		send(request);
	}
	 ...
	 * <code>Unbind</code> PDU, waits for <code>UnbindResp</code> and then closes
	 * the connection.<br>
	 ...
	 *
	 * @return response PDU if successfully unbound, i.e. if still not bound after
	 *         call to the method; null if unbind was not successful, i.e. no
	 *         response has been received for unbind PDU
	 *
	 * @exception java.io.IOException
	 *              exception during communication
	 * @exception PDUException
	 ...
	 * @see UnbindResp
	 */
	final public UnbindResp unbind() throws ValueNotSetException, TimeoutException, PDUException, IOException, WrongSessionStateException {
	 ...
	 * @exception ValueNotSetException
	 *              optional param not set but requested (this exception doesn't
	 *              apply for genericNack)
	 * @see GenericNack
	 */
	final public void genericNack(GenericNack response) throws ValueNotSetException, TimeoutException, IOException, WrongSessionStateException {
		checkState(response);
		try {
			respond(response);
	 ...
	 * @see GenericNack
	 */
	final public void genericNack(int commandStatus, int sequenceNumber) throws ValueNotSetException, TimeoutException, IOException, WrongSessionStateException {
	...
	/**
	 * Submits provided <code>SubmitSM</code> PDU to SMSC and returns response to
	 * the submission.
	 * <p>
	 ...
	 * @see SubmitSMResp
	 */
	final public SubmitSMResp submit(SubmitSM request) throws ValueNotSetException, TimeoutException, PDUException, IOException, WrongSessionStateException {
		checkState(request);
	 ...
	 */
	final public SubmitMultiSMResp submitMulti(SubmitMultiSM request) throws ValueNotSetException, TimeoutException, PDUException, IOException,
	...
	/**
	 * Submits provided <code>DeliverSM</code> PDU to SMSC and returns response to
	 * the submission.
	 * <p>
	 ...
	 * @see DeliverSMResp
	 */
	final public DeliverSMResp deliver(DeliverSM request) throws ValueNotSetException, TimeoutException, PDUException, IOException, WrongSessionStateException {
		checkState(request);
	 ...
	 * @see DataSMResp
	 */
	final public DataSMResp data(DataSM request) throws ValueNotSetException, TimeoutException, PDUException, IOException, WrongSessionStateException {
		checkState(request);
	 ...
	 * @see QuerySMResp
	 */
	final public QuerySMResp query(QuerySM request) throws ValueNotSetException, TimeoutException, PDUException, IOException, WrongSessionStateException {
		checkState(request);
	 ...
	 * @see CancelSMResp
	 */
	final public CancelSMResp cancel(CancelSM request) throws ValueNotSetException, TimeoutException, PDUException, IOException, WrongSessionStateException {
		checkState(request);
		return (CancelSMResp) send(request);
	}

	/**
	 * Replaces previously submitted message by sending <code>ReplaceSM</code> PDU
	 * to SMSC and returns response to the replace.
	 * <p>
	 ...
	 * @see ReplaceSMResp
	 */
	final public ReplaceSMResp replace(ReplaceSM request) throws ValueNotSetException, TimeoutException, PDUException, IOException, WrongSessionStateException {
		checkState(request);
	 ...
	 * @see EnquireLinkResp
	 */
	final public EnquireLinkResp enquireLink(EnquireLink request) throws ValueNotSetException, TimeoutException, PDUException, IOException,
			WrongSessionStateException {
		checkState(request);
	 ...
	 * As the <code>EnquireLink</code> PDU doesn't contain any parameters axcept
	 * of header, there is might be no need to provide the PDU as a parameter to
	 * the <code>enquireLink</code> method.<br>
	 ...
	 * @see EnquireLinkResp
	 */
	final public EnquireLinkResp enquireLink() throws ValueNotSetException, TimeoutException, PDUException, IOException, WrongSessionStateException {
	 ...
	 */
	final public void alertNotification(AlertNotification request) throws ValueNotSetException, TimeoutException, PDUException, IOException,
			WrongSessionStateException {
		checkState(request);
		send(request);
	}
	 ...
	 * @see ReceiverBase
	 */
	final public PDU receive() throws UnknownCommandIdException, TimeoutException, NotSynchronousException, PDUException, IOException {
		if (!asynchronous) {
	 ...
	 * @see ReceiverBase
	 */
	final public PDU receive(long timeout) throws UnknownCommandIdException, TimeoutException, NotSynchronousException, PDUException, IOException {
		PDU pdu = null;
	...
	/**
	 * Returns <code>Receiver</code> object created for receiving PDUs from SMSC.
	 *
	 ...
	 * length, <code>GenericNack</code> PDU is sent back to SMSC automatically to
	 * report the error and null is returned. If the timeout for receiving PDU set
	 * in the <code>Receiver</code> object expires, null is returned.
	 * <p>
	 *
	 * @param request
	 ...
	 */
	final private Response send(Request request) throws ValueNotSetException, TimeoutException, PDUException, IOException {
	 ...
	 * id's don't match then if the <code>pdu</code>'s command id is generic_nack,
	 * then the expected response is set the command id of generic_nack and
	 * command id, command status and sequence number of the received
	 * <code>pdu</code> and this way transformed response is returned: the class
	 * mathces, but the command id says, that the PDU is generic nack.<br>
	 ...
	 * @return either the received PDU or expected response transformed to generic
	 *         nack.
	 * @see #send(Request,boolean)
	 */
	 ...
	 * @param sequenceNumber
	 *          to allow the other party to match the generic ack with their sent
	 *          request
	 * @exception java.io.IOException
	...
	private void safeGenericNack(int commandStatus, int sequenceNumber) throws IOException {
		try {
			...
			// event.write(e,
			// "Ignoring unexpected exception caught sending generic nack.");
		}
	}
	...
	/**
	 * Returns the current state of the session. It's public to allow the user to
	 * check the state of the session if necessary.
	 * 
	 ...
	 * Note that the states are bit values in integer, so there can be a "set" of
	 * states required, the session must be in one of the states required. The
	 * checking can be turned off, see <a
	 * href="#disableStateChecking()">disableStateChecking</a>.
	 * 
	 * @param requestedState
	 *          the state(s) in which the session is expected to be; if it is not,
	...
	public void checkState(int requestedState) throws WrongSessionStateException {
		if (stateChecking) {
			// debug.write(DSESS,
			// "checking state current=0x" + Integer.toHexString(state) +
			// " requested esme=0x" + Integer.toHexString(requestedState));
	...
	/**
	 * Checks if the session's state allows sending the PDU provided. If not, then
	 * exception <code>WrongSessionStateException</code> is thrown. For each state
	 * there is only a subset of PDUs which can be sent over the session. For
	 * example, if the session is bound as a receiver, it cannot submit messages,
	 * but it can send enquire link, generic nack etc. This method checks the PDU
	 * type (command id) aganst matrix of allowed states. The checking can be
	 * turned off, see <a href="#disableStateChecking()">disableStateChecking</a>.
	 * 
	 * @param pdu
	...
	/**
	 * Checks if the session is in the state which is required by the parameter.
	 * Note that this method doesn't throw an exception rather it returns false if
	 * the session is not in one of the provided states.
	 * 
	 * @param requestedState
	 ...
	 * that this method doesn't throw an exception rather it returns false if the
	 * pdu is not alloewd in the current session state.
	 * 
	 * @param pdu
	 *          the pdu which has to be checked if it's allowed in the current
	 *          state
	...
	private static void initialiseStateMatrix() {
		...
		addValidState(mcStateMatrix, Data.BIND_TRANSMITTER_RESP, STATE_TRANSMITTER | STATE_RECEIVER | STATE_TRANSCEIVER); // STATE_OPENED);
		...
		addValidState(mcStateMatrix, Data.BIND_TRANSCEIVER_RESP, STATE_TRANSMITTER | STATE_RECEIVER | STATE_TRANSCEIVER); // STATE_OPENED);
	...
	/**
	 * God, I would never think that to keep unbind synchronous in an asynchronous
	 * enviroment would be so funny. Here is the replacement listener which
	 * encapsulates the original listener and hunts for the unbind pdu. Good luck,
	 * you source code reader!<br>
	 * The problem is that we want to return a response from session's unbind()
	 * even if the session is asynchronous, i.e. all pdus from the smsc are passed
	 * to an implementation of <code>ServerPDUEventListener</code> event
